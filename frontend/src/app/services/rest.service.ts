...
export class RestService {
  ...
  public get(url: string, options: any = {}): Observable<any> {
    ...
    if (options.noMessage) {
      headers = headers.set("X-PO-No-Message", options.noMessage)
    }
    if (options.screenLock) {
      headers = headers.set("X-PO-Screen-Lock", options.screenLock)
    }
  ...
  public post(url: string, body: any, ...args: { [header: string]: string }[]) {
    ...
    // Só adiciona headers se não for FormData
    if (!(body instanceof FormData)) {
      headers = Object.assign(new HttpHeaders(), this.headers)
      args.some((arg) => {
        const param = arg as any
        if (param.noMessage && headers) {
          headers = headers.set("X-PO-No-Message", param.noMessage)
        }
        if (param.screenLock && headers) {
          headers = headers.set("X-PO-Screen-Lock", param.screenLock)
        }
      })
    }
    ...
    return this.http.post(environment.baseUrl + url, body, {
      headers, // undefined aqui permite Angular setar multipart/form-data corretamente
    })
  }
  ...
  public deleteAll(
   ...
   /**
   * @description
   * Função http.delete simples que aceita apenas o ID na URL.
   */
   public delete(
    url: string,
    ...args: { [header: string]: string }[]
